...
export default function CommentsPage() {
  ...
  // 确认采集: 评论采集=单文章更新流程(接口统一 /collect/update, 其它开关全关只开评论参数)
 ...
 capture_4metrics: false, capture_read: false,
          ...
          while ((i2 = buf.indexOf("\n\n")) !== -1) {
            ...
            try {
              ...
              if (d.type === "log" && d.msg) {
                ...
                // 前端统计: 解析后端写库日志 [async:xx] 识别评论N条, 写入M条(一级X 二级Y)
                // 写入M=当轮增量(累加); 一级X/二级Y=会话累计(直接设值); 速度由 effect 按时间窗计算
                const mk = d.msg.match(/识别评论(\d+)条, 写入\s*(\d+)条/);
                if (mk) {
                  setCcCount((c) => c + Number(mk[2]));
                  const mlv = d.msg.match(/一级(\d+) 二级(\d+)/);
                  if (mlv) { setCcCount1(Number(mlv[1])); setCcCount2(Number(mlv[2])); }
                }
              } else if (d.type === "done") {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* 导入进度弹窗 */}
      <Modal destroyOnHidden mask={{ closable: false }} title="正在导入" open={importing} footer={null} closable={false} width={400}>
      ...
      {/* 评论采集弹窗: 确认阶段 -> 采集进行中 */}
        ...
        stats={[
          { label: "开始时间", value: ccStartTime },
          { label: "采集评论数", value: `${ccCount}` },
          { label: "一级评论数", value: `${ccCount1}` },
          { label: "二级评论数", value: `${ccCount2}` },
          { label: "采集速度", value: `${ccSpeed.toFixed(1)} 条/秒` },
        ]}
        logs={ccLogs}
        onCancel={() => { if (ccStarted) { stopCc(); return; } closeCc(); }}
        onConfirm={confirmCommentCollect}
        onCloseFinish={closeCc}
      />
    </div>
  );
}